...
impl VectorBenchmark {
    ...
    fn generate_vector_types(&self) -> Vec<(&'static str, VectorData)> {
        vec![
            (
                "F32",
                VectorData::F32(generators::random_f32(self.vector_size)),
            ),
            (
                "F64",
                VectorData::F64(generators::random_f64(self.vector_size)),
            ),
            (
                "F16",
                VectorData::F16(generators::random_f16(self.vector_size)),
            ),
            (
                "BF16",
