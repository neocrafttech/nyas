    ...
    ID: Debug + Clone + Hash + Eq + PartialEq + Send + Sync + 'static,
{
    ...
    pub fn insert(&mut self, point: &VectorPoint<ID>) {
        ...
        self.locks
            .insert(Arc::clone(&point.id), Arc::new(Mutex::new(())));
        ...
        let (_, visited) = self.greedy_search(
            &self.start_node.clone().unwrap(),
            &point.vector,
            1,
            self.l_build,
        );
        ...
        self.graph
            .insert(Arc::clone(&point.id), out_neighbors.clone());
    ...
    pub fn greedy_search(
        ...
        let mut result_with_dist: Vec<_> = visited
            ...
            .filter_map(|id| {
                self.points
                    .get(id)
                    .map(|p| (id, p.distance_to_vector(query, self.metric)))
            })
            .collect();
        ...
        let top_k: Vec<Arc<ID>> = result_with_dist
            .into_iter()
        ...
        while !candidates.is_empty() {
            let (best_idx, _) = candidates
                ...
                .filter_map(|(i, cid)| {
                    self.points
                        .get(cid)
                        .map(|cp| (i, point.distance(&cp, self.metric)))
                })
                .reduce(
                    || (usize::MAX, f64::MAX),
    ...
    pub fn delete(&mut self, delete_list: &[Arc<ID>]) {
        ...
        for point_ref in self.graph.iter() {
            ...
            let deleted_neighbors: Vec<_> = neighbors
                .iter()
                .filter(|&n| delete_set.contains(n))
                .cloned()
                .collect();

            if !deleted_neighbors.is_empty() {
                let mut candidates: HashSet<_> = neighbors
                    .iter()
                    .filter(|&n| !delete_set.contains(n))
                    .cloned()
                    .collect();
