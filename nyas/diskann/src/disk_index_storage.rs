...
impl DiskIndexStorage {
    ...
    pub async fn open(index_name: &str) -> Result<Self, std::io::Error> {
        let (data_file_path, graph_file_path) = index_file_paths(index_name);
    ...
    pub async fn insert(&mut self, mem_index: &InMemIndex) -> io::Result<()> {
        let (data_file_path, graph_file_path) = index_file_paths(&self.index_name);
    ...
    async fn write_graph(&mut self, mem_index: &InMemIndex) -> io::Result<()> {
        ...
        for r in points_vec {
            ...
            let target_offset = BLOCK_SIZE as u64 + (point.id as u64 * aligned_node_size as u64);
            file.seek(SeekFrom::Start(target_offset)).await?;
            file.write_all(&node_buffer).await?;
        }
    ...
    async fn write_data(&mut self, mem_index: &InMemIndex) -> io::Result<()> {
        ...
        for r in points_vec {
            ...
            let target_offset = point.id as u64 * aligned_vector_size as u64;
            file.seek(SeekFrom::Start(target_offset)).await?;
    ...
    pub async fn read_node(&self, id: u32) -> Result<(DiskNode, VectorData), std::io::Error> {
        let index_file = self.index_file.as_ref().ok_or_else(|| {
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "No index file")
        })?;
        let meta = self
        ...
        // 1. Read Graph Node (neighbors)
        let graph_offset = BLOCK_SIZE as u64 + (id as u64 * meta.max_node_size);
        let mut graph_bytes = vec![0u8; meta.max_node_size as usize];

        let mut gf = index_file.graph_file.lock().await;
        gf.seek(SeekFrom::Start(graph_offset)).await?;
        gf.read_exact(&mut graph_bytes).await?;
        drop(gf);

        let size_bytes: [u8; 8] = graph_bytes[0..8].try_into().unwrap();
        let size = u64::from_le_bytes(size_bytes);
        if size == 0 {
            return Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Empty node slot"));
        }

        let node_bytes = &graph_bytes[8..8 + size as usize];
        let node: DiskNode = from_bytes::<DiskNode, rancor::Error>(node_bytes)
            .map_err(|e| std::io::Error::other(e.to_string()))?;

        // 2. Read Vector Data
        let vector_offset = id as u64 * meta.point_size;
        let mut vector_bytes = vec![0u8; meta.point_size as usize];

        let mut df = index_file.data_file.lock().await;
        df.seek(SeekFrom::Start(vector_offset)).await?;
        df.read_exact(&mut vector_bytes).await?;
        drop(df);

        let v_size_bytes: [u8; 8] = vector_bytes[0..8].try_into().unwrap();
        let v_size = u64::from_le_bytes(v_size_bytes);

        let v_bytes = &vector_bytes[8..8 + v_size as usize];
        let vector: VectorData = from_bytes::<VectorData, rancor::Error>(v_bytes)
            .map_err(|e| std::io::Error::other(e.to_string()))?;
    ...
    pub async fn greedy_search_ssd(
        ...
        let Ok((start_node, start_vector)) = self.read_node(start_node_id).await else {
            return;
        };

        let start_point = VectorPoint::new(start_node.id, start_vector.clone());
        candidates.push(SearchCandidate {
            point_id: start_node.id,
        ...
        node_cache.insert(start_node.id, (start_node, start_vector));

        let lut = self.pq.as_ref().map(|pq| pq.compute_lut(query));
        ...
        while let Some(current) = candidates.pop() {
            ...
            let (node, vector) = if let Some(cached) = node_cache.remove(&current.point_id) {
                cached
            ...
            for neighbor_id in neighbors {
                ...
                if let Some(pq) = &self.pq
                    && let Some(lut) = &lut
                    && let Some(codes) = self.codes.get(&neighbor_id)
                {
                    distance_estimated = Some(pq.estimate_distance(lut, codes));
                }

                let distance = if let Some(dist) = distance_estimated {
                    dist
                } else if let Some((_, v)) = node_cache.get(&neighbor_id) {
                    v.distance(query, metric_type)
                } else if let Ok((neighbor_node, neighbor_vector)) =
                    self.read_node(neighbor_id).await
                {
                    let dist = neighbor_vector.distance(query, metric_type);
                    node_cache.insert(neighbor_id, (neighbor_node, neighbor_vector));
                    dist
                } else {
                    continue;
                };
    ...
    fn create_pq(&mut self, mem_index: &InMemIndex) {
        ...
        for r in points_vec {
            ...
            self.codes.insert(point.id, codes);
        }
